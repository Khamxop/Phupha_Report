...
if (isset($payment) && is_array($payment)) {
    foreach ($payment as $payRow) {
        ...
        // ຖ້າ Base_Currency ເປັນ LAK ແມ່ນໃຫ້ຂ້າມໄປເລີຍ (ບໍ່ເອົາມາໄລ່)
        if ($baseCurrency === 'LAK') {
            continue;
        }
...
// Calculate Total_Customize from Payment table
$totalProduct = 0;
$last7Product = 0;
$prev7Product = 0;
...
if (isset($payment) && is_array($payment)) {
    foreach ($payment as $pay) {
        ...
        $financialGroups['income']['items'][] = [
            ...
            'payAmount' => $payAmount,
            'discount' => $discount,
            'Total_Customize' => $pay['Total_Customize'] ?? '',
...
if (is_array($incomeexpenses)) {
    foreach ($incomeexpenses as $inex) {
        ...
        $financialGroups[$groupKey]['items'][] = [
            ...
            'Total_Customize' => $inex['Total_Customize'] ?? '',
